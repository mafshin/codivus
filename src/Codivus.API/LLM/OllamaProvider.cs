...
public class OllamaOptions
{
    ...
    /// </summary>
    public string DefaultModel { get; set; } = "codellama:7b-instruct";
...
public class OllamaProvider : ILlmProvider
{
    ...
    /// <param name="options">Ollama options</param>
        ...
        _retryPolicy = Policy
            ...
            .WaitAndRetryAsync(
                ...
                (exception, timeSpan, retryCount, context) =>
                {
                    _logger.LogWarning(
                        exception,
                        "Error calling Ollama API (Attempt {RetryCount} of {MaxRetries}). Retrying in {RetryTimeSpan}...",
                        retryCount,
                        _options.MaxRetries,
                        timeSpan);
                });
    ...
    public async Task<IEnumerable<string>> GetAvailableModelsAsync()
    {
        try
        {
            return await _retryPolicy.ExecuteAsync(async () =>
            {
                ...
                try
                {
                    // Try to parse as standard Ollama format
                    if (rootElement.TryGetProperty("models", out var models))
                    {
                    ...
                    // If we couldn't parse the structure or no models were found, add fallback model
                    if (modelNames.Count == 0)
                    {
                        modelNames.Add(_options.DefaultModel);
                    }
                }
                catch (Exception ex)
                {
        ...
        catch (Exception ex)
        {
            ...
            return new List<string> { _options.DefaultModel };
        }
    }
    ...
    public async Task<IEnumerable<CodeIssue>> AnalyzeCodeAsync(string code, string filePath, ScanConfiguration configuration, Guid scanId)
    {
        try
        {
            return await _retryPolicy.ExecuteAsync(async () =>
            {
                ...
                var model = string.IsNullOrEmpty(configuration.LlmModel) ? _options.DefaultModel : configuration.LlmModel;
                ...
                var requestBody = new
                {
                    ...
                    system = "You are a code analysis tool that identifies issues, vulnerabilities, and improvements in code. " +
                ...
                var responseText = responseJson.GetProperty("response").GetString() ?? string.Empty;
        ...
        {
            _logger.LogError(ex, "Error analyzing code with Ollama");
    ...
    public async Task<string> GenerateFixSuggestionAsync(CodeIssue issue, string code)
    {
        try
        {
            return await _retryPolicy.ExecuteAsync(async () =>
            {
                ...
                var requestBody = new
                {
                    ...
                    top_p = _options.TopP,
                    max_tokens = _options.MaxTokens,
                    system = "You are a code improvement assistant that provides fixes for code issues. " +
        ...
        {
            _logger.LogError(ex, "Error generating fix suggestion with Ollama");
